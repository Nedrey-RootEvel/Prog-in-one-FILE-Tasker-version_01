...
class Storage {
    ...
    public function addUser($username, $password, $permissions) {
        foreach ($this->users_data as $user) {
            if ($user['username'] === $username) {
                return false;
            }
    ...
    public function updateUser($id, $username, $password, $permissions) {
        foreach ($this->users_data as &$user) {
            if ($user['id'] == $id) {
    ...
    public function addTab($name) {
        ...
        foreach ($this->tabs_data as $tab) {
            if (($tab['order'] ?? 0) > $max_order && $tab['type'] !== 'trash') {
    ...
    public function getItemsByTab($tab_id) {
        $tab = null;
    ...
    public function getTitles($tab_id = null) {
        $items = $this->getItemsByTab($tab_id ?? 1);
        $titles = [];
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><span class="badge"><?=htmlspecialchars($item['author'] ?? 'guest')?></span></td>
                <td>
...
if (isset($_POST['login'])) {
    ...
    if ($user) {
        $_SESSION['user'] = $user;
        $client_id = substr(md5($_SERVER['REMOTE_ADDR']), 0, 8);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($act == 'getTitles') echo json_encode($db->getTitles($_POST['tab_id']));
    ...
    if ($act == 'addTab') echo json_encode(['ok'=>$db->addTab($_POST['name'])]);
    ...
    if ($act == 'addUser') echo json_encode(['ok'=>$db->addUser($_POST['username'], $_POST['password'], json_decode($_POST['permissions'], true))]);
    if ($act == 'updateUser') echo json_encode(['ok'=>$db->updateUser($_POST['id'], $_POST['username'], $_POST['password'], json_decode($_POST['permissions'], true))]);
...
$items = $db->getItemsByTab($current_tab);
$titles = $db->getTitles($current_tab);

$current_tab_type = 'all';
foreach ($tabs as $tab) {
    if ($tab['id'] == $current_tab) {
        $current_tab_type = $tab['type'] ?? 'custom';
        break;
    }
}
?>
    ...
    <div class="header-fixed">
        <div class="header-top">
            ...
            <div class="header-right">
                <?php if ($db->checkPermission($current_user, 'add')): ?>
                <button class="btn" onclick="openModal()">➕ Добавить</button>
                <?php endif; ?>
                
                <?php if ($current_tab_type !== 'trash'): ?>
            ...
            <div class="tabs-list">
                ...
                <a href="?tab=<?=$tab['id']?>" class="<?=$tab_class?>">
                    ...
                    <?php if ($tab['type'] == 'trash'): ?>
                        (<?=count($db->getItemsByTab($tab['id']))?>)
                    <?php endif; ?>
                    <?php if (!$tab['is_default'] && $current_user['role'] == 'admin' && $tab['type'] == 'custom'): ?>
                    <span class="delete-tab-btn" onclick="deleteTab(event, <?=$tab['id']?>, '<?=htmlspecialchars($tab['name'])?>')">×</span>
                    <?php endif; ?>
                </a>
                <?php endforeach; ?>
            </div>
            <?php if ($current_user['role'] == 'admin'): ?>
            <button class="add-tab-btn" onclick="addTab()">➕ Добавить вкладку</button>
            <?php endif; ?>
        </div>
    </div>
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th style="width: 200px;">ДАТЫ / СТАТУС</th>
                        ...
                        <td colspan="6" style="padding: 0;">
                            <div class="empty-state">
                                <p style="font-size: 16px; margin-bottom: 10px;">
                                    <?php if ($current_tab_type == 'trash'): ?>🗑️ Корзина пуста<?php else: ?>📭 Нет записей<?php endif; ?>
                                </p>
                            </div>
                        </td>
                    </tr>
                    <?php else: ?>
                        ...
                        <tr id="row-<?=$item['id']?>" class="<?=$row_class?>">
                            ...
                            <td>
                                ...
                                <?php if ($replies_count > 0 && $current_tab_type !== 'trash' && $can_comment): ?>
                            ...
                            <td>
                                <?php if ($current_tab_type !== 'trash'): ?>
                                    <div style="margin-bottom: 4px;">
                                        ...
                                        <?php if (!empty($item['updated'])): ?>
                                            <span style="color: #2196F3; margin-left: 8px;">✏️ <?=$item['updated']?></span>
                                        <?php endif; ?>
                                    </div>
                            ...
                            <td style="text-align: center;">
                                <div class="actions">
                                    <?php if ($current_tab_type !== 'trash'): ?>
                                        ...
                                        <?php endif; ?>
                                        <?php if ($db->checkPermission($current_user, 'edit', $item['author'] ?? null)): ?>
                                        <button class="btn edit btn-sm" onclick="editItem(<?=$item['id']?>)">
                                            ✏️
                                        </button>
                                        <?php endif; ?>
                                        <?php if ($db->checkPermission($current_user, 'delete', $item['author'] ?? null)): ?>
        ...
        <div class="modal-content">
            ...
            <form onsubmit="saveReply(event)">
                ...
                <div style="background: #e9ecef; padding: 10px; border-radius: 4px; margin-bottom: 15px; font-size: 13px;">
                    <strong>📌 <?=htmlspecialchars($current_user['username'])?>, ответ на:</strong><br>
    ...
    <script>
        ...
        function saveTab(e) {
            ...
            fetch('', {
                ...
                body: 'action=addTab&name=' + encodeURIComponent(name)
            })
            .then(r => r.json())
        ...
        function loadUsers() {
            ...
            .then(users => {
                ...
                html += '<thead><tr><th>ID</th><th>Имя</th><th>Роль</th><th>Права</th><th>Действия</th></tr></thead><tbody>';
        ...
        function openUserModal() {
            ...
            document.getElementById('perm_add').checked = true;
            ...
            .then(replies => {
                ...
                if (replies.length > 0) {
                    ...
                    replies.forEach(reply => {
                        ...
                        html += '<span class="reply-meta">' + reply.created_at + ' от ' + escapeHtml(reply.author) + '</span>';
                        html += '</div>';
        ...
        function exportExcel() {
            let data = [];
            data.push(['№', 'Заголовок', 'Описание', 'Автор', 'Выполнено', 'Актуально', 'Дата создания', 'Дата изменения']);
            ...
            rows.forEach(row => {
                if (!row.querySelector('.empty-state')) {
                    ...
                    if (cols.length > 1) {
                        ...
                        let statusCells = cols[4]?.querySelectorAll('.status-item') || [];
                        ...
                        let dates = cols[4]?.querySelector('div:first-child')?.innerText.split('✏️') || [];
                        let created = dates[0]?.replace('📅', '').trim() || '';
                        let updated = dates[1]?.trim() || '';
                        data.push([num, title, desc, author, completed, actual, created, updated]);
                    }
                }
            });
